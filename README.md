# Interview App — AI Interview Platform
...
### 3. Создать .env для backend (секреты из AWS Secrets Manager)
```bash
cd interview-app-backend

# Автоматически достать секреты:
...
### 4. Создать .env.local для frontend
```bash
