...
function isCreatePendingAction(
  ...
  return action.type === 'create_interview' || action.type === 'create_questions';
}
...
function questionEquals(
  ...
  return left.key === right.key && JSON.stringify(left) === JSON.stringify(right);
}
...
export function applyCreatePendingActionQuestionOverride(
  ...
  if (override.questions.length === 0 || override.questions.length > stored.questions.length) {
    return null;
  }

  const storedByKey = new Map(stored.questions.map((question) => [question.key, question]));
